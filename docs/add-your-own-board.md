...
`src/clapper_ai/displays/terminal.py`:

```python
"""Prints each grid to the terminal. The simplest possible DisplaySink."""

from clapper_ai.core.codes import CODE_TO_CHAR
...
line, one config value. The Brain, inputs, and LLMs are untouched.
...
- If your board supports fewer tile codes than the full map, pass your own
  `allowed` set to `Brain(...)` so `validate_grid` protects you.
